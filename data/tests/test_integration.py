...
def _import_module_with_fallback(names):
    for name in names:
        try:
...
vehicle_rental = _import_module_with_fallback(['vehicle_rental', 'models.vehicle_rental'])
individual_user = _import_module_with_fallback(['individual_user', 'models.individual_user', 'individualuser', 'models.individualuser'])
corporate_user = _import_module_with_fallback(['corporate_user', 'models.corporate_user', 'corporateuser', 'models.corporateuser'])
staff_user = _import_module_with_fallback(['staff_user', 'models.staff_user', 'staffuser', 'models.staffuser'])
car = _import_module_with_fallback(['car', 'models.car'])
rental_period = _import_module_with_fallback(['rental_period', 'models.rental_period', 'rentalperiod', 'models.rentalperiod'])

if vehicle_rental is None or individual_user is None or corporate_user is None or staff_user is None or car is None or rental_period is None:
    missing = []
    if vehicle_rental is None:
        missing.append('vehicle_rental')
    if individual_user is None:
        missing.append('individual_user')
    if corporate_user is None:
        missing.append('corporate_user')
    if staff_user is None:
        missing.append('staff_user')
    if car is None:
        missing.append('car')
    if rental_period is None:
        missing.append('rental_period')
    raise ImportError(f"Could not import required modules: {', '.join(missing)}")
...
def full_system():
    ...
    system.add_users(CorporateUser("CO001", "Alice", "alice@corp.com", "Corp", "CR123", "123 Main Street, Auckland", "pass123"))
...
def test_corporate_user_rental_workflow(full_system):
    ...
    # Login
    user = full_system.authenticate_user("CO001", "pass123")
    assert user is not None

    # Rent vehicle
    period = RentalPeriod("01-01-2026", "06-01-2026")  # 6 days
    success = full_system.rent_vehicles("CAR002", "CO001", period)
    assert success is True
...
def test_staff_view_all_rentals(full_system):
    ...
    success2 = full_system.rent_vehicles("CAR002", "CO001", period2)
    ...
    user2 = full_system._find_renter_by_id("CO001")
